...
[Financial Forecasting](#-financial-time-series-forecasting) •
...
- **Domain-Optimized Financial Endpoint**: Built-in `/v1/forecast/financial` endpoint handling OHLCV bars, log-return stationarity, and tail-risk Value at Risk (VaR) quantification.
...
### 3. Financial Time Series (`POST /v1/forecast/financial`)

A specialized endpoint tailored for quantitative trading platforms, auto-traders, and market analysis. It accepts historical OHLCV bars, applies log-difference stationarity transformations, forecasts forward trajectories, and returns:
- **Expected Return (bps)**: Basis point return derived from median trajectory.
- **Uncertainty Spread**: 80% credible interval ($q_{90} - q_{10}$).
- **Downside Risk (VaR proxy)**: $10^{\text{th}}$ percentile lower bound.
- **Directional Signal**: `bullish`, `bearish`, or `neutral`.

#### Request:
```bash
curl -X POST "http://localhost:8088/v1/forecast/financial" \
  -H "Content-Type: application/json" \
  -d '{
    "symbol": "AAPL",
    "bars": [
      {"timestamp": "2026-09-17T10:00:00Z", "open": 182.1, "high": 183.0, "low": 181.8, "close": 182.7, "volume": 1250000},
      {"timestamp": "2026-09-17T10:05:00Z", "open": 182.7, "high": 183.5, "low": 182.4, "close": 183.2, "volume": 1410000}
    ],
    "horizon_bars": 12,
...
```json
{
  ...
  "horizon_bars": 12,
  "current_price": 183.20,
  "predicted_price_p50": 184.45,
  "expected_return_bps": 68.23,
  "signal": "bullish",
  "confidence": 0.84,
  "uncertainty_spread_bps": 42.10,
  "downside_var_p10_bps": -18.50,
  "trajectory": {
    "p10": [183.10, 183.15, ...],
    "p50": [183.35, 183.50, ...],
    "p90": [183.60, 183.90, ...]
...
### 1. No Financial, Investment, or Trading Advice
> [!CAUTION]
> **IMPORTANT FINANCIAL NOTICE:**
> The software, models, endpoints (including `/v1/forecast/financial`), projected return estimates, quantile bands, Value at Risk (VaR) metrics, and directional indicators (`bullish`, `bearish`, `neutral`) generated by **TimesFM Serve** are provided strictly for **technological research, academic evaluation, and informational purposes only**.
>
> - Nothing contained in this repository, software, or API output constitutes financial, investment, legal, or tax advice.
> - **TimesFM Serve** does not recommend, endorse, or solicit the purchase or sale of any securities, equities, commodities, cryptocurrencies, or financial instruments.
> - Algorithmic trading and financial market speculation carry substantial risk of capital loss. Past statistical performance and zero-shot foundation model predictions do not guarantee future market behavior.
> - **Under no circumstances shall `enlotec`, its directors, employees, or contributors be held liable for any direct or indirect financial losses, missed profits, trading damages, or capital depreciation arising from the use of this software or its outputs.** Users assume 100% sole responsibility for their trading and investment decisions.

### 2. Software License & Limitation of Liability
...
### 3. Google TimesFM Model Weights & Intellectual Property
